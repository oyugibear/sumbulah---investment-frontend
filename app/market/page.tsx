...
import { MOCK_MARKET_DATA, MOCK_STOCK_HOLDINGS, ALL_SAUDI_SECURITIES } from "@/lib/mock-data"
...
export default function MarketPage() {
  ...
  const holdings = MOCK_STOCK_HOLDINGS.filter((h) => h.shares && h.shares > 0)
  ...
  const sectors = useMemo(() => {
    const uniqueSectors = new Set(ALL_SAUDI_SECURITIES.map((s) => s.sector))
    return Array.from(uniqueSectors).sort()
  }, [])

  // Filter securities
  const filteredSecurities = useMemo(() => {
    return ALL_SAUDI_SECURITIES.filter((security) => {
      const matchesSearch =
        searchQuery === "" ||
  ...
  }, [searchQuery, selectedSector, selectedType, shariahOnly])
