...
export default function HomePage() {
  ...
  const { state, addDeposit, addGoal, updateSettings, logAnalytics } = useApp()
  ...
  useEffect(() => {
    ...
    const reachedMilestone = milestones
      .filter((m) => state.totalInvested >= m.amount && !celebratedMilestones.includes(m.amount))
      .sort((a, b) => b.amount - a.amount)[0] // Get the highest uncelebrated milestone
  ...
  }, [state.totalInvested, celebratedMilestones, milestonesLoaded]) // Added milestonesLoaded dependency

  // Handle milestone dismissal
  const handleMilestoneDismiss = () => {
    if (milestone) {
      // Add this milestone to the celebrated list
      const newCelebratedMilestones = [...celebratedMilestones, milestone.amount]
  ...
  }, [celebratedMilestones, state.totalInvested])
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      {showMilestone && milestone && (
        <MilestoneCelebration milestone={milestone} onDismiss={handleMilestoneDismiss} />
      )}
      ...
      <div className="mx-auto max-w-lg px-4 py-8 space-y-6 animate-in fade-in duration-500">
        ...
        <header className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-emerald-950 dark:text-emerald-50">
              {t("home.title")}, {(state as any).user?.name || "Guest"} 👋
            </h1>
        ...
        {/* Payday Nudge */}
        {state.nextAutoDeposit && state.nextAutoDeposit.amount > 0 && (
